...
export async function verifyPassword(password, stored) {
  try {
    // Panoya kopyalarken sona düşen boşluk/satır sonu, base64 çözümünü bozup
    // girişi SESSİZCE imkânsız kılıyordu. Yapılandırma değeri kırpılır.
    ...
    // KRİTİK (QA B1): saklanan özet kırpılmışsa (kopyala-yapıştır sırasında son
    // '$' sonrası kaybolursa) 0 bit türetip 0 baytı 0 baytla kıyaslamak HER
    // parolayı doğru sayardı. SHA-256 çıktısı her zaman 32 bayttır; salt boşsa
    // da reddet. Türetilecek uzunluk saklanan değere DEĞİL, sabite bağlanır.
    if (expected.length !== 32 || salt.length === 0) return false;
    const key = await crypto.subtle.importKey('raw', enc.encode(password), 'PBKDF2', false, ['deriveBits']);
    const bits = await crypto.subtle.deriveBits(
      { name: 'PBKDF2', hash: 'SHA-256', salt, iterations }, key, 256,
    );
 ...
 * Cloudflare kenarı HER ZAMAN CF-Connecting-IP koyar. X-Forwarded-For'a
 * DÜŞMEYİZ (QA N3): saldırgan onu istediği gibi değiştirip her istekte farklı
 * kova üretebilirdi. Kenar başlığı yoksa herkes tek 'unknown' kovasını paylaşır
 * — bu bilinçli: sınırlama gevşemez, sıkılaşır.
 */
...
/**
 * { blocked, remaining } döner. KAPALI ÇÖKER (QA N2): KV yoksa ya da okuma
 * hata verirse engelle. Gerekçe: KV zaten yoksa veri ucu 503 döner ve panel
 * hiçbir şey göstermez, yani kapalı çökmenin erişilebilirlik maliyeti sıfırdır;
 * buna karşılık açık çökmek tek kaba-kuvvet frenini kaldırır ve her denemede
 * 210.000 turluk PBKDF2 hesabı yaptırarak CPU tüketim vektörü açardı.
 */
export async function checkRateLimit(kv, key, nowMs = Date.now()) {
  // reason: 'unavailable' → sayaç okunamıyor (beklemek DÜZELTMEZ)
  //         'too_many'    → gerçekten sınır aşıldı (beklemek düzeltir)
  if (!kv) return { blocked: true, remaining: 0, reason: 'unavailable' };
  try {
    const raw = await kv.get(key);
    const rec = raw ? JSON.parse(raw) : null;
    if (!rec || nowMs - rec.first > RL_WINDOW_SEC * 1000) return { blocked: false, remaining: RL_MAX_ATTEMPTS };
    return { blocked: rec.n >= RL_MAX_ATTEMPTS, remaining: Math.max(0, RL_MAX_ATTEMPTS - rec.n), reason: 'too_many' };
  } catch (e) {
    return { blocked: true, remaining: 0, reason: 'unavailable' };
  }
}
