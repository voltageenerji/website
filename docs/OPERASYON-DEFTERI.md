...
| **Yönetim paneli** | ✅ Canlı ve yapılandırıldı 2026-08-26 | `/panel` — kullanıcı adı + parola ile tüm teklif talepleri; arama, sayfalama, CSV. Doğrulandı: kasten yanlış parola → 401 `invalid_credentials` (yapılandırma sağlam), `Cache-Control: no-store, private`, `X-Robots-Tag: noindex, nofollow, noarchive`, etkin `frame-ancestors 'none'`, `/lib/` ve `/tools/` servis dışı. Ortam değişkenleri: ADMIN_USER (Text), ADMIN_PASS_HASH + ADMIN_SESSION_SECRET (Secret). KV: LEADS → voltage-leads. **Cihaz kaybında:** ADMIN_SESSION_SECRET döndür → tüm oturumlar anında düşer. |
| **Raporlar: 5 dönem** | ✅ Kurulu 2026-08-26 | Haftalık / aylık / 3 aylık / 6 aylık / yıllık — takvim bazlı, bitişik pencereler (çakışma ve boşluk yok). Cron'lar `.github/workflows/lead-reports.yml`. Rapor yanıtı artık firma adı DÖNDÜRMEZ (olay kaydı §3.1). |
...
| voltan.com.tr → voltage.com.tr 301 — **domainler Pages projesinde "Verifying" durumunda** (2026-08-26 tespiti) | Sahip/eklenti | GSC "Alternate page with proper canonical" bulgusunun kapanması. YENİ BİLGİ: voltan.com / voltan.com.tr ve www'lu halleri Pages projesinde Custom domain olarak eklenmiş ama "Verifying" aşamasında takılı. **Active olurlarsa repodaki `_redirects` kuralları kendiliğinden çalışır ve zone kuralı GEREKMEZ.** Önce doğrulamanın neden tamamlanmadığına bakılmalı (genelde DNS kaydı). Sonrasında GSC'de "Validate fix". SEO değeri kaybı YOK (canonical konsolidasyonu çalışıyor) — hijyen işi. |
...
2. **encazip.com — DÜZELTME (06.09): site KAPALI.** Sahip teyit etti; DNS doğruladı (www çözümlenmiyor,
   kök alan adı Squarespace park IP'sinde). Arama motorunun ve yapay zekânın gösterdiği "1.000 kWh eşiği"
   ve kurucu bilgisi **ölü sitenin dizin hayaleti** — ~2022 tarihli önbellek (1.000 kWh o yılın serbest
   tüketici limitiydi). Orchestrator ilk okumada ENOTFOUND sinyalini yanlış yorumladı.
   **Düzeltilemez, sahiplenilemez; dizin yeniden taradıkça kendiliğinden düşer.** Bizim kaldıracımız:
   canlı, doğrulanabilir birinci taraf sinyalleri güçlü tutmak (kurumsal sayfa, schema kimlikleri, llms.txt).

**Sonuç:** kimlik tarafı güçlü ve iyileşiyor; konu otoritesi henüz yok (5 haftalık site — beklenen).
Kaldıraç: **elektrikpaketleri.com** (DNS'te Cloudflare arkasında yaşıyor; sahibe ve sandbox'a açılmıyor —
